...
- **T6 実装範囲:** `src/fake-adapter.ts`（コンパイル済み CLI を argv 配列・`shell:false` で起動し stdin 1 行で `record.recall` のみ送る薄い fake。入力 32768B・stdout 上限・壁時計 timeout 既定 5000ms・100..10000ms clamp を常時適用。disabled/利用不可/破損/非ゼロ起動/timeout/不正応答/予算超過は全て固定文言 `ordinary response without memory` + 固定 `code` の memoryless 縮退に写像し、stderr/stdout/例外文・クエリ文・記憶本文を結果に複写しない。成功時は封筒・`recallId`・件数上限・行 shape を厳密検証し、要求 scope のみ露出する。引用は当該 recall の record ID のみ（`isCitationExposed`）。LLM 呼出し・ネットワーク・Companion 実装なし）+ `tests/acceptance.test.ts`（A1–A13 を domain fixture + CLI subprocess + fake adapter で自動検査。seeded fake データのみ、実ユーザデータ不使用）+ `package.json` の `acceptance` script（`npm run build && node --test dist/tests/acceptance.test.js`）。
- **テスト（本ブランチで実測）:** `npm test`（tsc + node:test、tests 87・pass 86・fail 0・skipped 1）— T1–T4 一式 + T6 A1–A13（13 件）。`npm run acceptance`（tests 13・pass 13・fail 0）。`npm run smoke`（有効/未知/人手 op 封筒の walkthrough に成功）。唯一の skip は実 TTY 対話確認（手動のみ、自動化不可）。実 TTY の `review` 全文表示・`approve/reject/archive` の `yes` 手入力・負荷下 timeout 観察・kill-during-commit 観察は自動化の主張対象外であり、未実施のまま残す（`docs/acceptance-manifest.md` の manual-only 欄）。
