...
## 17. 採用記録（ユーザ承認済み・T3 時点）

- **承認 (approved):** §14 のうち T3 範囲（承認済み `records` のみを対象とする `record.recall` + 露出監査）の採用。実装リポジトリ = 本ディレクトリ（remind）を維持し、Companion 変更なし（U11 未解決のまま）。
- **pin（T3 時点・変更なし）:** Node.js v24.12.0 / npm 11.6.2 / `engines >=24.12.0 <25` / TypeScript 5.9.3 / SQLite は Node 組込 `node:sqlite`（runtime 依存ゼロ）。T1–T2 の記録を維持する。
- **T3 実装範囲:** `record.recall` のみ。厳密 params（未知 field・bounds・コードポイント・制御文字・正準ミリ秒時刻）+ T2 と同一正規化（body/query/tag は NFKC/trim/畳み込み/ASCII fold、link は casefold なし）+ 本文リテラル部分一致（ワイルドカードなし、SQL LIKE 不使用）+ タグ exact ALL + リンク exact + `createdAt` に対する `since` inclusive / `until` exclusive + 同一認可 scope の `active` のみ + 固定順序 `createdAt DESC, id ASC` の上位 limit 選択（正常範囲の選択は `LIMIT_EXCEEDED` にしない）+ 先頭 `snippetMaxCp` コードポイントの前方投影 + `truncated` + 応答全体 UTF-8 予算（超過は切詰めず `LIMIT_EXCEEDED`、部分露出なし）。応答は一意 `recallId` + `items[{id, snippet, truncated, tags, createdAt}]`。選択読取りと `audit` 1 行（`bytes=応答バイト, limitN`）+ `exposures` 行（返却 ID 毎・`snippetBytes/truncated/limitN`、本文・クエリ・snippet 全文なし）を単一トランザクションで原子記録し、応答確定後にコミット、監査/露出失敗・DB 満杯/破損は `STORE_UNAVAILABLE` で fail closed（無監査の成功を作らない）。DB 上限は起動時検査 + 想起書込み前の `PRAGMA page_count/page_size` + 実ファイル/`-wal`/`-shm` 合計の pragmatic best-effort 検査で `STORE_UNAVAILABLE` とし、正確なバイト保証は主張しない（ページ粒度・WAL チェックポイント遅延のため近似）。T4（訂正/archive）・T6（fake adapter）は実装せず、テストは T2 承認フローで seeded した records と状態 setup のみを用いる。
- **テスト:** `npm test`（tsc + node:test、**46 件合格・1 件 skip**）— T1–T2 一式（T3 に伴う既存期待の更新: `record.recall` 空 params は `BAD_REQUEST`、`record.correct-request` が T4 延期の `NOT_IMPLEMENTED` 代表に）+ T3 回帰 10 件（承認前漏洩なし・失効 scope 拒否、リテラル正規化 `%_`/`"`、タグ ALL・リンク・時刻・順序・同点 id 順・limit、Unicode 切断と応答バイト、空想起の監査、監査/エラーのクエリ不在、recallId 一意と露出突合、監査強制失敗の rollback、`LIMIT_EXCEEDED` の無露出、CLI 永続 seeded E2E）。唯一の skip は実 TTY 対話確認（手動のみ）。`npm run smoke` は strict 有効 recall（空 DB で `ok:true` 空 items）/未知/人手 op の walkthrough に成功。T4–T6（訂正/archive・保持文書・fake adapter・A1–A13）は T3 の範囲外であり、本節は T4+ の実装・受入を主張しない。
- **未解決のまま:** U1–U11（U11 連携契約を含む）の正式決定、A1–A13 受入（T6）、T4–T6 ドメイン。本文書の実装済み主張は T1–T3 範囲に限定し、§1–§13 の planning-only 位置づけを維持する。

## 18. 採用記録（ユーザ承認済み・T4 時点）

- **承認 (approved):** §14 のうち T4 範囲（`record.correct-request` + 訂正承認の原子 supersede + 人手 `archive` + 監査）の採用。実装リポジトリ = 本ディレクトリ（remind）を維持し、Companion 変更なし（U11 未解決のまま）。
- **pin（T4 時点・変更なし）:** Node.js v24.12.0 / npm 11.6.2 / `engines >=24.12.0 <25` / TypeScript 5.9.3 / SQLite は Node 組込 `node:sqlite`（runtime 依存ゼロ）。T1–T3 の記録を維持する。
- **T4 実装範囲:** `record.correct-request`（`candidate.create(kind=correction, supersedes=recordId)` 短縮形。`kind` 指定時は `correction` のみ、`ttlSec` 不可。作成時に対象の存在・同一 scope・`active` を検査し、承認時に条件付き UPDATE で原子再検査）+ `candidate.create(kind=correction)` の対象 gate（`supersedes` 必須、対象不在 `NOT_FOUND`、scope 不一致 `FORBIDDEN_SCOPE`、非 active `CONFLICT`）+ 訂正承認（新 record 追記 + 候補 `approved` + 旧 record `active → superseded` を同一トランザクションで行い、条件付き UPDATE の更新行数で競合承認の勝者を 1 件に絞る。敗者は `CONFLICT` で全巻き戻し、敗者候補は `candidate` のまま残る）+ 人手 `archive`（TTY 両 stream 必須・`yes` 対話確認・`--id/--scope/--idempotency-key/--reason-code USER_ARCHIVED` 必須、token 不要。条件付き `active → archived` flip。終端・復帰なし・物理削除なし。自動化 JSON 経由は `FORBIDDEN`）。`record.recall` は `status=active` のみ対象のため superseded/archived は想起に出ない。訂正承認前は旧 record が想起対象のまま残る。冪等 replay（同一キー+同一 params は `deduplicated:true`、params 改変は `CONFLICT`、確定済み replay は対象移動後も安定再生・`EXPIRED` を使わない）と scope 認可再検査・予算 gate（超過は無孤児 `LIMIT_EXCEEDED`）・本文なし監査を T2–T3 と同一契約で適用する。人手 `review/approve/reject/archive` は stdin/stdout 両 TTY 必須（stdout リダイレクトによる本文・token の文件取り込みを防ぐ）。T5（保持文書）・T6（fake adapter・A1–A13）は実装せず、テストは直接 store 呼び出し + CLI JSON 封筒 + `isHumanTty`/`validateRequest` の単体検査を用いる。
- **テスト:** `npm test`（tsc + node:test、**68 件合格・1 件 skip**）— T1–T3 一式（T4 に伴う既存期待の更新: `record.correct-request` 空 params は `BAD_REQUEST`、訂正作成の対象不在は `NOT_FOUND`、未実装ドメイン扱いの廃止）+ T4 回帰 8 件（訂正承認の原子 supersede と連鎖、訂正競合の勝者 1 件と敗者 rollback、archive 競合の両方向、scope 分離と想起不可視、冪等 replay と params 改変 `CONFLICT`、失敗時の無孤児、superseded/archived の想起除外、JSON/nonTTY archive 拒否）。唯一の skip は実 TTY 対話確認（手動のみ）。`npm run smoke` は strict 有効 recall（空 DB で `ok:true` 空 items）/未知/人手 op の walkthrough に成功。T5–T6（保持文書・fake adapter・A1–A13）は T4 の範囲外であり、本節は T5+ の実装・受入を主張しない。
- **未解決のまま:** U1–U11（U11 連携契約を含む）の正式決定、A1–A13 受入（T6）、T5–T6 ドメイン。本文書の実装済み主張は T1–T4 範囲に限定し、§1–§13 の planning-only 位置づけを維持する。

## 19. 採用記録（ユーザ承認済み・T5+T6 時点）

- **承認 (approved):** §14 のうち T5 範囲（保持既定の文書化）+ T6 範囲（fake adapter + A1–A13 受入）の採用。実装リポジトリ = 本ディレクトリ（remind）を維持し、Companion 変更なし（U11 未解決のまま）。本節は T5 文書（`docs/retention.md`）と T6 コード（`src/fake-adapter.ts`・`tests/acceptance.test.ts`・`package.json` の `acceptance` script）をまとめて採用記録とし、§§15–18 の履歴は書き換えない。
- **pin（T5+T6 時点・変更なし）:** Node.js v24.12.0 / npm 11.6.2 / `engines >=24.12.0 <25` / TypeScript 5.9.3 / SQLite は Node 組込 `node:sqlite`（runtime 依存ゼロ）。T1–T4 の記録を維持する。
- **T5 実装範囲（文書のみ・コード変更なし）:** `docs/retention.md` が §14.9 の採用済み既定を宣言する（TTL は承認猶予のみ・全終端状態を保持・purge/`DELETE` 経路なし・`admin.export` 未実装・削除 API なし・消去保証なし・監査は本文なし・`dbMaxBytes` は pragmatic best-effort・満杯/破損時は記憶なし通常応答に縮退・復旧は手動手順）。`docs/acceptance-manifest.md` が A1–A13 の自動/手動対応表を宣言する（自動は `tests/acceptance.test.ts`、手動は実 TTY のみ）。U7（保持・エクスポート・削除方針）・U11（Companion 連携契約）の正式決定は未解決のまま残す。
- **T6 実装範囲:** `src/fake-adapter.ts`（コンパイル済み CLI を argv 配列・`shell:false` で起動し stdin 1 行で `record.recall` のみ送る薄い fake。入力 32768B 既定・stdout 8192B 既定・壁時計 timeout 既定 5000ms を常時適用し、有限整数の 1024..65536B / 100..10000ms 範囲外は縮退（不正形 BAD_REQUEST、超過 LIMIT_EXCEEDED、limit<1 は BAD_REQUEST）で silent clamp しない。`killSignal:SIGKILL` で SIGTERM 無視子を強制終了。disabled/利用不可/破損/非ゼロ起動/timeout/不正応答/予算超過は全て固定文言 `ordinary response without memory` + 固定 `code` の memoryless 縮退に写像し、stderr/stdout/例外文・クエリ文・記憶本文を結果に複写しない。stdout は UTF-8 fatal デコード（不正は縮退）。成功時は厳密封筒（`ok:true` は `code:OK`・`message:"ok"`・boolean `deduplicated`・未知 top-level なし・LF 1 行のみ）・`recallId`・件数上限・行 shape（`createdAt,id,snippet,tags,truncated` のみ、id 一意・制御/孤立サロゲートなし・`createdAt` 正規 UTC ms RFC3339・tags<=5・snippet<=200cp、過大設定は非互換として縮退）を検証し、要求 scope のみ露出する。引用は当該 recall の record ID のみ（`isCitationExposed`、構造ゲートであり認証ではない）。LLM 呼出し・ネットワーク・Companion 実装なし）+ `src/store.ts` の stored-bodyHash 完全性ガード（body/bodyHash 改竄は approve/reject とも CONFLICT、txn 内外で再読検証）+ `tests/acceptance.test.ts`（A1–A13 + A8b 厳密封筒/予算フィクスチャを domain fixture + CLI subprocess + fake adapter で自動検査。A3 は全 token 結合フィールド改竄を網羅。seeded fake データのみ、実ユーザデータ不使用）+ `package.json` の `acceptance` script（`npm run build && node --test dist/tests/acceptance.test.js`）。
- **テスト（本ブランチで実測）:** `npm test`（tsc + node:test、tests 88・pass 87・fail 0・skipped 1）— T1–T4 一式 + T6 関連。`npm run acceptance`（tests 14・pass 14・fail 0：A1–A13 + A8b）。`npm run smoke`（有効/未知/人手 op 封筒の walkthrough に成功）。唯一の skip は実 TTY 対話確認（手動のみ、自動化不可）。実 TTY の `review` 全文表示・`approve/reject/archive` の `yes` 手入力・負荷下 timeout 観察・kill-during-commit 観察は自動化の主張対象外であり、未実施のまま残す（`docs/acceptance-manifest.md` の manual-only 欄）。
- **未解決のまま:** U1–U11（U7 保持方針・U11 連携契約を含む）の正式決定、実 TTY 手動確認、production Companion 統合（行わない）、vector 索引・consolidation・background jobs・`used_memory` reheat（MVP 外）。本文書の実装済み主張は T1–T6 範囲に限定し、§1–§13 の planning-only 位置づけを維持する。
