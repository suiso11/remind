# T6 acceptance manifest A1–A13 (automated vs manual)

Automated suite: `tests/acceptance.test.ts` (`npm run acceptance` =
`npm run build && node --test dist/tests/acceptance.test.js`).
...
| A3 | Token/immutability binding: wrong token is `CONFLICT`, no record | `A3 approval immutability binding` | `tests/t2.test.ts`, `t2-review-fixes` token binding | real-TTY token transcription (hand-run) |
